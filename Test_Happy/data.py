...
def plot_correlation(ax, df, y_column, y_label):
    ...
    if y_label == 'Freiheit':
        ax.scatter(train[y_column], train['happiness_score'], alpha=0.3, color='lightgray', label='Train (2019–2023)')
    ax.plot(x_line, y_line, color='black', linewidth=2, label='Regression (2024–2025)')
    ax.set_title(f'{y_label} vs. Happiness', fontweight='bold')
...
fig.suptitle('World Happiness Report – Korrelationen (2024-2025)\nTrain: 2019–2023   |   Test: 2024–2025', fontsize=14, fontweight='bold')
...
faktoren = {
    'explained_log_gdp_per_capita': 'BIP',
    'explained_social_support': 'Soziale Unterstützung',
    'explained_healthy_life_expectancy': 'Lebenserwartung',
    'explained_freedom': 'Freiheit',
    'explained_generosity': 'Generosität',
    'explained_corruption': 'Korruption',
...
order = ['Happiness', 'BIP', 'Soziale Unterstützung', 'Lebenserwartung', 'Freiheit', 'Korruption', 'Generosität']
corr_matrix = corr_matrix.loc[order, order]
mask_triangle = np.triu(np.ones_like(corr_matrix, dtype=bool), k=1)
corr_matrix_masked = corr_matrix.mask(mask_triangle)

im = ax.imshow(corr_matrix_masked, cmap='RdYlGn', vmin=-1, vmax=1)
...
for i in range(len(corr_matrix)):
    for j in range(len(corr_matrix)):
        val = corr_matrix_masked.iloc[i, j]
